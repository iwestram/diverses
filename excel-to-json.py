...
#Excel-Dateinamen abfragen
excel_file = input("Pfad der Excel-Datei angeben: ")

# Excel-Datei einlesen
# excel_file = 'nsg-normal.xlsx'
try:
    df = pd.read_excel(excel_file)
except FileNotFoundError:
    print(f"Die Datei '{excel_file}' wurde nicht gefunden. Bitte überprüfen Sie den Pfad und versuchen Sie es erneut.")
except Exception as e:
    print(f"Ein Fehler ist aufgetreten: {e}")

# NSG oder LSG XML abfragen
xmlname = input("Bitte Namen der Raumbezug-Datei angeben (z.B. 'lsg.xml'): ")
xmltype = input("Bitte Schutzgebiets-Typ angeben (z.B. 'lsg'): ")
...
for index, row in df.iterrows():
    ...
    if not pd.isna(row['A']):
        wkt = wkt_for_title(xmlname, xmltype, row['A'])
        if wkt:
